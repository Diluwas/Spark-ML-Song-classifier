...
public abstract class CommonLyricsPipeline implements LyricsPipeline {
    ...
    public GenrePrediction predict(final String unknownLyrics) {
        ...
        if (Arrays.asList(predictionsDataset.columns()).contains("probability")) {
            ...
            return new GenrePrediction(getGenre(prediction).getName(), probability.apply(0), probability.apply(1), probability.apply(2), probability.apply(3), probability.apply(4), probability.apply(5), probability.apply(6), 0D);
        }
    ...
    Dataset<Row> readLyrics() {
        String directoryPath = lyricsTrainingSetDirectoryPath + "/output";
        Dataset input = readLyricsForGenre(directoryPath, Genre.ROCK)
                .union(readLyricsForGenre(directoryPath, Genre.POP))
                .union(readLyricsForGenre(directoryPath, Genre.COUNTRY))
                .union(readLyricsForGenre(directoryPath, Genre.JAZZ))
                .union(readLyricsForGenre(directoryPath, Genre.HIP_HOP))
                .union(readLyricsForGenre(directoryPath, Genre.BLUES))
                .union(readLyricsForGenre(directoryPath, Genre.REGGAE));
    ...
    private Dataset<Row> readLyricsForGenre(String inputDirectory, Genre genre) {
        Dataset<Row> lyrics = readLyrics(inputDirectory, genre.name().toUpperCase() + ".csv");
